...
function ret = loss(model, data, wd_coefficient)
  ...
  wd_loss = sum(model_to_theta(model).^2)/2*wd_coefficient; % weight decay loss. very straightforward: E = 1/2 * wd_oeffecient * theta^2
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  ret.hid_to_class = model.hid_to_class * 0 + wd_coefficient * model.hid_to_class;
end
